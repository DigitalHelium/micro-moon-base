class_name MetalBuilding

class MetalBuildingClass extends Building.BuildingClass:
	func init_part_class(settings: Dictionary) -> BuildingPart.BuildingPartClass:
		...
		if type == BuildingPart.Type.Dynamite:
			return DynamitePart.new(atlas_coord_value)
		return null
